									...
									<tr>
										...
										<table border="1">
											...
											<xsl:for-each select="liste/mot">
												<xsl:sort select="*[name()=$tri_demande]"  order="ascending"/>
												...
												<tr>
													...
													<td><!--<a href="CreateConcordance.php?tri=trad_mot&amp;phrase={$nom_s}&amp;texte={$nom_t}">--><xsl:value-of select="texte"/><!--</a>--></td>	
													<td><xsl:value-of select="num_phrase"/></td>	
													<td align="right"><xsl:value-of select="mot_gauche"/></td>
													...
													<td bgcolor="grey"><a href="#" onClick="window.open('ViewOneSimilarity.php?similarity={$similarity}&amp;color={$color}&amp;file={$nom_t}&amp;sentence={$nom_s}&amp;mot={$mot}','Fiche','toolbar=no,status=no,width=900 ,height=600,scrollbars=yes,location=no,resize=yes,menubar=yes')"><xsl:value-of select="mot_cours"/></a></td>	
